...
export default function MateriPaiClient() {
  ...
  const handleGenerate = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${apiBase}/api/tools/materi-pai`, {
        method: 'POST',
        headers: {
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          ...
          >
            ...
            <option value="Pendidikan Agama Islam (Umum)">Pendidikan Agama Islam (Umum)</option>
          </select>
        </div>

        <div className="space-y-2">
          <label className="block font-label-md text-label-md text-on-surface">Topik Bahasan</label>
          <input
            type="text"
            value={topik}
            onChange={(e) => setTopik(e.target.value)}
            disabled={isGenerating}
            placeholder="Contoh: Rukun Syarat Mandi Wajib, atau Sejarah Bani Umayyah"
