...
export default function KitabKuningClient() {
  ...
  const handleGenerate = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${apiBase}/api/tools/kitab-kuning`, {
        method: 'POST',
        headers: {
    ...
    <form onSubmit={handleGenerate} className="space-y-6">
      <div className="space-y-4">
        <div className="space-y-2">
          <label className="block font-label-md text-label-md text-on-surface">Teks Arab Gundul</label>
          <textarea
            value={teksArab}
            onChange={(e) => setTeksArab(e.target.value)}
            disabled={isGenerating}
            rows={4}
            placeholder="Contoh: الحمد لله رب العالمين..."
