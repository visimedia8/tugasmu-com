...
export default function PenjelasHaditsClient() {
  ...
  const handleGenerate = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${apiBase}/api/tools/penjelas-hadits`, {
        method: 'POST',
        headers: {
    ...
    <form onSubmit={handleGenerate} className="space-y-6">
      <div className="space-y-4">
        <div className="space-y-2">
          <label className="block font-label-md text-label-md text-on-surface">Teks Hadits / Kata Kunci Tema</label>
          <input
            type="text"
            value={hadits}
            onChange={(e) => setHadits(e.target.value)}
            disabled={isGenerating}
            placeholder="Contoh: Innamal a'malu binniyat (Hadits niat)"
