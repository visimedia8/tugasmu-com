...
export default function TafsirQuranClient() {
  ...
  const handleGenerate = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${apiBase}/api/tools/tafsir-quran`, {
        method: 'POST',
        headers: {
    ...
    <form onSubmit={handleGenerate} className="space-y-6">
      <div className="space-y-4">
        <div className="space-y-2">
          <label className="block font-label-md text-label-md text-on-surface">Surat dan Ayat Ke-</label>
          <input
            type="text"
            value={ayat}
            onChange={(e) => setAyat(e.target.value)}
            disabled={isGenerating}
            placeholder="Contoh: Al-Baqarah ayat 183"
