...
export default function NahwuShorofClient() {
  ...
  const handleGenerate = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${apiBase}/api/tools/nahwu-shorof`, {
        method: 'POST',
        headers: {
    ...
    <form onSubmit={handleGenerate} className="space-y-6">
      <div className="space-y-4">
        <div className="space-y-2">
          <label className="block font-label-md text-label-md text-on-surface">Kalimat Arab yang Ingin Di-I'rab</label>
          <input
            type="text"
            value={kalimat}
            onChange={(e) => setKalimat(e.target.value)}
            disabled={isGenerating}
            placeholder="Contoh: ضرب زيد كلبا"
