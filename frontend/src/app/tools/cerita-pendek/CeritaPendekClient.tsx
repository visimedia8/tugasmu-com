...
export default function CeritaPendekClient() {
  const { data: session } = useSession();

  const [tema, setTema] = useState('');
  const [tokoh, setTokoh] = useState('');
  const [gayaBahasa, setGayaBahasa] = useState('Pengalaman Pribadi (Sudut pandang Aku)');
  ...
  const handleGenerate = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${apiBase}/api/tools/cerita-pendek`, {
        method: 'POST',
        headers: {
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          ...
          <input
            ...
            placeholder="Contoh: Liburan ke rumah nenek di desa melihat sapi"
        ...
        <div className="space-y-2">
          ...
          <input
            type="text"
            value={tokoh}
            onChange={(e) => setTokoh(e.target.value)}
            disabled={isGenerating}
            placeholder="Contoh: Budi, atau 'Aku'"
            className="w-full px-3.5 py-3 rounded-xl bg-surface-container-low text-on-surface font-body-md text-body-md focus:bg-surface-container-lowest focus:ring-2 focus:ring-primary-container outline-none transition-all disabled:opacity-50"
          />
        </div>
        ...
        <div className="space-y-2">
          ...
          >
            <option value="Pengalaman Pribadi (Sudut pandang Aku)">Pengalaman Pribadi (Sudut pandang Aku)</option>
            <option value="Dongeng Hewan (Fabel)">Dongeng Hewan (Fabel)</option>
            <option value="Petualangan Seru (Sudut pandang Dia)">Petualangan Seru (Sudut pandang Dia)</option>
          </select>
        </div>
      </div>
