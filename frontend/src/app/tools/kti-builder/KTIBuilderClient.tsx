...
export default function KTIBuilderClient() {
  ...
  const [metode, setMetode] = useState('Kualitatif Deskriptif');
  ...
  const handleGenerate = async (e?: React.FormEvent) => {
    ...
    if (!topik.trim()) {
      setError('Tuliskan topik atau judul KTI yang ingin dibuat.');
      return;
    }
    ...
    try {
      ...
      const res = await fetch(`${apiBase}/api/tools/kti-builder`, {
        method: 'POST',
        headers: {
    ...
    <form onSubmit={handleGenerate} className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          <label className="block font-label-md text-label-md text-on-surface">Topik / Judul Karya Tulis Ilmiah (KTI)</label>
          <input
            type="text"
            value={topik}
            onChange={(e) => setTopik(e.target.value)}
            disabled={isGenerating}
            placeholder="Contoh: Efektivitas Penggunaan Botol Plastik Bekas sebagai Sistem Hidroponik..."
        ...
        <div className="space-y-2">
          <label className="block font-label-md text-label-md text-on-surface">Jenis Penelitian / Metode</label>
          <select
            value={metode}
          ...
          >
            <option value="Kualitatif Deskriptif">Kualitatif Deskriptif (Observasi & Wawancara)</option>
            <option value="Kuantitatif (Eksperimen / Survei)">Kuantitatif (Eksperimen / Angket Kuesioner)</option>
            <option value="Studi Pustaka (Literature Review)">Studi Pustaka (Menganalisis Buku/Jurnal)</option>
            <option value="Pengembangan Produk (R&D)">Pengembangan Produk (Membuat Inovasi Baru)</option>
          </select>
        </div>
      </div>
      ...
      >
        {isGenerating ? (
          <>
            ...
            <span>Menyusun Kerangka KTI...</span>
          </>
        ) : (
          <>
            <span className="material-symbols-outlined text-[20px]">science</span>
            <span>Buat Kerangka KTI (BAB I - III)</span>
          </>
        )}
      </button>
