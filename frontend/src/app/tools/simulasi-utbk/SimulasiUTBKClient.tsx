...
export default function SimulasiUTBKClient() {
  const { data: session } = useSession();

  const [subtes, setSubtes] = useState('');
  const [tingkatKesulitan, setTingkatKesulitan] = useState('HOTS (High Order Thinking Skills)');
  ...
  const handleGenerate = async (e?: React.FormEvent) => {
    ...
    if (!subtes) {
      setError('Pilih subtes SNBT terlebih dahulu.');
      return;
    }
    ...
    try {
      ...
      const res = await fetch(`${apiBase}/api/tools/simulasi-utbk`, {
        method: 'POST',
        headers: {
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <select
            ...
            className="w-full px-3.5 py-3 rounded-xl bg-surface-container-low text-on-surface font-body-md text-body-md focus:bg-surface-container-lowest focus:ring-2 focus:ring-primary-container outline-none transition-all disabled:opacity-50 appearance-none"
          >
            <option value="Dasar (Review Konsep)">Dasar (Review Konsep)</option>
            <option value="Menengah (Mirip UTBK Asli)">Menengah (Mirip UTBK Asli)</option>
            <option value="HOTS (High Order Thinking Skills)">HOTS (Lebih Sulit dari UTBK Asli)</option>
          </select>
        </div>
      </div>
      ...
      >
        {isGenerating ? (
          <>
            ...
            <span>Membuat Soal Try Out...</span>
          </>
        ) : (
          <>
            <span className="material-symbols-outlined text-[20px]">school</span>
            <span>Generate Simulasi Soal</span>
          </>
        )}
      </button>
